  ...
  class Database
  {
    ...
    public function insert($prenom,$nom,$email,$type,$login,$password,$photo,$password1){
        $erreur = "faux";
        ...
        if ($count > 0) {
          ...
          $sql= "INSERT INTO utilisateur( prenom, nom, email, type, login, password, photo,monpass) VALUES (:prenom,:nom,:email,:type,:login,:password,:photo,:password1)";
          $stmt= $this->conn->prepare($sql);
          $stmt->execute(['prenom'=>$prenom,'nom'=>$nom,'email'=>$email,'type'=>$type,'login'=>$login,'password'=>MD5($password),'photo'=>$photo,'password1'=> $password1]);
          $count = $stmt->rowCount();
          if($count >0){
            $erreur ="faux";
          }else{
    ...
    public function update($id,$prenom, $nom, $email, $type, $login, $password, $photo, $password1){
      $sql= "UPDATE utilisateur SET prenom = :prenom,nom = :nom,email = :email,type = :type,login = :login ,password = :password ,photo =:photo,monpass =:password1 WHERE iduser = :id";
      $stmt = $this->conn->prepare($sql);
     $stmt->execute(['prenom'=>$prenom,'nom'=>$nom,'email'=>$email,'type'=>$type,'login'=>$login,'password'=> MD5($password),'photo'=>$photo,'id'=>$id,'password1' => $password1]);
     return true;
    }
